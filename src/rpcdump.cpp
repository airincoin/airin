...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Airin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"airinaddress\"\n"
            "\nEncrypts a private key corresponding to 'airinaddress'.\n"
            "\nArguments:\n"
            "1. \"airinaddress\"   (string, required) The airin address for the private key (you must hold the key already)\n"

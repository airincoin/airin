...
*Setup instructions for a gitian build of Airin using a Debian VM or physical system.*
...
Clone the git repositories for airin and gitian and then checkout the airin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the airin repository
...
To build Airin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the airin repository.
...
```bash
    ...
    From https://github.com/airin/airin
...
For example:
```bash
URL=https://github.com/konez2k/airin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/airin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/airin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/airin-build.assert
```
...
[airin/gitian.sigs](https://github.com/airin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
